 ...
 *
 */
angular.module("MallApp.AreaSelectorBookingGoodsCity", [])
    ...
    .directive("ajlBookingAreaSelector", function () {/*提供数据源url 剩下的交给它自己处理吧  这里会提供许多事件钩子 你可以挂在你所关心地事件  希望你在写列表的事件簿超过1分钟了...*/
        return {
            restrict: 'ECMA',
            ...
            controller: function ($scope, baseService) {
                $scope.$watch("reset", function (newValue,oldValue) {
                    if(newValue){
                        console.log("resetreset",newValue,oldValue,$scope.data.catch);
                        _.each($scope.data.catch, function (item) {
                            console.log(item);
                            $scope.clickUtils.removeItem(item);
                        })
                        $scope.data.disabledParam={};
                        $scope.data.query.provinceList=[];
                        $scope.data.query.provinceId="";
                        $scope.data.query.cityList=[];
                        $scope.data.query.cityId="";
                        $scope.data.query.communityName="";
                        $scope.data.query.groundCommunityCityList=[];
                        $scope.data.queryArea.provinceList=[];
                        $scope.data.queryAreaParam.provinceId="";
                ...
                var postParam={
                    url:'/ejiazi-merchant/citys/queryOpeningCitiesListForGoods.json',
                    method:'get',
                    pid:0,
                    goodsId:$scope.goodsId
                };
                console.log("goodsId",$scope.goodsId);
                ...
                console.log("**********************",angular.copy($scope.disabledParam),$scope.data,$scope.disabledParam);
                $scope.area={
                ...
                $scope.clickUtils={
                    getCitys:function(province){
                        ...
                        console.log("cityParamcityParamcityParamcityParam",cityParam)
                        ...
                        baseService.getData(cityParam).then(function (data) {
                            console.log(_.values($scope.data.stateCatch[3]))
                                ...
                                if(preCityIdList){
                                    ...
                                    _.each(data.data.data, function (city) {
                                ...
                                province.cityList=data.data.data;
                        },function (error) {
                            console.log(error)
                        })
                    },
                    ...
                    queryCommunity: function () {
                        ...
                        var communityListParam={
                            url:'/ejiazi-merchant/booking/queryCommunitiesList.json',
                            method:'get',
                                ...
                                if(param.paramType=="community"){
                                    ...
                                    if(item.paramType=="circle"){
                                        if(item.provinceId==param.provinceId&&item.cityId==param.cityId&&item.districtId==0){
                                            ...
                                            if(param.realPath.indexOf(item.realPath)==0||(item.districtId==param.districtId&&param.ringId<=item.ringId)){
                                                /*小区被省市区包含不添加小区*/
                                                param.checked=false;
                                                delete $scope.data.catch[param.catch_id];
                                            }
                                ...
                                if(param.paramType=="circle"&&param.districtId!=0){
                                    ...
                                    if(item.paramType=="community"){
                                        ...
                                        if(item.provinceId==param.provinceId&&item.cityId==param.cityId&&item.districtId==0){
                                            ...
                                            if(item.realPath.indexOf(param.realPath)==0||(item.districtId==param.districtId && item.ringId<=param.ringId)){
                                                item.checked=false;
                                                delete $scope.data.catch[item.catch_id];
                                                /*小区被省市区包含不添加小区*/
                                             //   return item;
                ...
                baseService.getData(postParam).then(function (data) {
                    if(!$scope.data.query.province){
                        $scope.data.query.provinceList=data.data.data;
                        $scope.data.provinceList=data.data.data;
                        $scope.data.queryArea.provinceList=data.data.data;
                ...
                $scope.$watch("data.query.provinceId", function (newValue, oldValue) {
                    ...
                    console.log("1",cityParam);
                    cityParam=_.extend(cityParam,$scope.otherQuery);
                    baseService.getData(cityParam).then(function (data) {
                        $scope.data.query.cityList=data.data.data;
                    },function (error) {
                        console.log(error)
                    })
                ...
                $scope.$watch("data.query.cityId", function (newValue, oldValue) {
                    ...
                    baseService.getData(areaParam).then(function (data) {
                        $scope.data.query.areaList=data.data.data;
                    },function (error) {
                        console.log(error)
                    })
                    var circleParam={
                    ...
                    baseService.getData(circleParam).then(function (data) {
                        $scope.data.query.circleList=data.data.data;
                    },function (error) {
                        console.log(error)
                    })
                })
                ...
                $scope.$watch("data.queryAreaParam.provinceId", function (newValue, oldValue) {
                    ...
                    console.log("2",cityParam);
                    cityParam=_.extend(cityParam,$scope.otherQuery);
                    baseService.getData(cityParam).then(function (data) {
                        $scope.data.queryArea.cityList=data.data.data;
                    },function (error) {
                        console.log(error)
                    })
                })
                ...
                $scope.$watch("data.queryAreaParam.cityId", function (newValue, oldValue) {
                    ...
                    areaParam=_.extend(areaParam,$scope.otherQuery);
                    baseService.getData(areaParam).then(function (data) {
                        $scope.data.queryArea.areaList=data.data.data;
                    },function (error) {
                        console.log(error)
                    })
                    var circleParam={
                        url:'/ejiazi-merchant/ring/queryOpeningRingListForGoods.json',
                        method:'get',
